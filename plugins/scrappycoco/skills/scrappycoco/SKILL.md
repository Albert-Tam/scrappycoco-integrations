...
If installed via npm, `npx --yes @scrappycoco/cli@0.8.11` is also supported.
...
For GEO, AEO, AI-visibility, or answer-engine citation work, first read [the GEO evidence workbook](references/geo-analysis.md). Use `web.ask_chatgpt`, `web.ask_copilot`, `web.ask_perplexity`, `web.ask_gemini`, `web.ask_google_ai_mode`, or `web.ask_grok`; inspect availability and never substitute engines.
Google AI Overviews use `web.search_web` with inspected AI-overview options. Explicit Cloro routes in `web.research` remain compatible.

For social video research, account critiques, hooks, micro-influencers, trends, Instagram/Meta ad inspiration or creator business contacts, first read the [social research cookbook](references/social-research.md). Inspect live availability; the calling AI adapts the recipe into `input.question` and evaluates the answer.
...
For a repeated-template batch, follow **Repeated-template batches** above. Reuse a representative result only if equivalent; otherwise include it later. For structured extraction and result reuse, follow [output handling](references/recipes.md#structured-output-and-result-reuse).
