...
REQUIRED_BODY_PHRASES = (
    ...
    "Catalog → Inspect → Run",
    "One URL does not always mean one bounded item",
    "initial direct Run made without prior Discovery",
    "next billable acquisition step must be Discovery",
    "do not try same-provider scroll",
    "Use a tested fallback before re-Discovering",
    ...
    "Default to two to four short sentences",
    "Do not include method narrative",
    "Maintain one task usage ledger",
    "End every completed handoff with one concrete, context-relevant question",
    "Do not use a generic “Anything else?” question",
    "## Other operations",
)
...
def validate_skill(skill_path: str) -> tuple[bool, str]:
    ...
    workbook_phrases = (
        ...
        "reports `40 of 166`",
        "next paid acquisition step must be a Discovery test",
        "a sequence of single-provider Runs is not Discover evidence",
        "--test",
        "--finalize",
        "default user-facing handoff is two to four short sentences",
        "finish with one concrete, context-relevant question",
        "When the user asks for an audit",
        "request and Discovery IDs",
        "A Discovery test is one gateway request",
        "Include failed billable attempts",
        "label the aggregate provisional",
    ...
    prompt_phrases = (
        ...
        "A single URL can be a multi-item collection",
        "default candidates for every available provider",
        "not another same-provider probe",
        "ledger of distinct request IDs and provider attempts",
        "sum only finalized exact response usage",
        "never count tool turns",
        "Keep the default user handoff to two to four short sentences",
        "end with one concrete next-task suggestion question",
