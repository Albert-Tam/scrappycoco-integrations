...
A single URL containing a directory, marketplace, search page, or other
multi-item collection is not automatically a bounded one-item task. If a page
reports `40 of 166`, or any other captured-versus-expected mismatch, the next
paid acquisition step must be a Discovery test on that same representative
input with `routing: "compare"`. Do not issue another direct Run with different
scrolls, waits, JavaScript, facets, formats, or the same provider first. The
comparison must preserve the default candidate for every available provider;
a sequence of single-provider Runs is not Discover evidence.
...
scrappycoco run web.extract_content --file request.json --json
```
...
evidence. If every compared candidate exposes the same source-side collection
cap, then inspect another catalog capability or a known public/static endpoint;
use direct Run only after that acquisition method is exact rather than uncertain.
...
scrappycoco discover --id DISCOVERY_ID --test \
...
## 5. Report a concise handoff and preserve audit evidence

The default user-facing handoff is two to four short sentences. Answer the
request directly, link or attach the useful output, mention only material gaps,
and finish with one concrete, context-relevant question proposing the natural
next task. Do not append a technical run report or ask a generic “Anything
else?” question.

Preserve the following evidence. When the user asks for an audit, debugging,
billing, or provenance—or a material failure requires explanation—report only
...
Keep a task-level ledger as requests complete. Record each distinct `request_id`,
the number of provider attempts inside that request, `usage.billing_status`,
`usage.unresolved_cost_count`, `usage.provider_cost_usd_exact`, and
`usage.payg_charge_usd_exact`. A Discovery test is one gateway request that may
contain several provider attempts, so report those counts separately. Include
failed billable attempts. For finalized rows, sum the exact decimal strings;
never substitute `attempt.estimated_cost_usd`, count tool turns, or report a
rounded guess. If any row is pending, reconciliation-pending, or unresolved,
label the aggregate provisional rather than presenting a final task total.

Default example:

```text
Done — I captured all 166 companies and saved them as CSV and JSON. One company
has no website, and seven do not list a team size; everything else is complete.

Would you like me to scrape the next batch?
```
